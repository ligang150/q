...
_preload_thread_lock = threading.Lock()


def _preload_one_model(model, config):
    """预抓取单个型号的产能数据"""
    try:
        sheet_id, start_row, capacity_col, limit_cell, row_count = config
        cache_key = f"{sheet_id}:{start_row}:{capacity_col}:{limit_cell}"
...
def _preload_all_models():
    ...
    for model, config in MODEL_CONFIG.items():
        sheet_id, start_row, capacity_col, limit_cell, row_count = config
        cache_key = f"{sheet_id}:{start_row}:{capacity_col}:{limit_cell}"
        if _get_preloaded_data(cache_key) is None:
            to_preload.append((model, config))

    if not to_preload:
        print("[preload] 所有型号缓存有效，跳过", flush=True)
        return
    ...
    with ThreadPoolExecutor(max_workers=4) as executor:
        futures = {executor.submit(_preload_one_model, model, config): model
                   for model, config in to_preload}
        for future in as_completed(futures):
            result = future.result()
            if result:
                success += 1

    print(f"[preload] 预抓取完成: {success}/{len(to_preload)} 个型号", flush=True)
