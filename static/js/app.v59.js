...
async function loadHistoryOrders(page = 1, forceRefresh = false) {
    historyCurrentPage = page;
    const ordersList = document.getElementById('historyOrdersList');
    if (!ordersList) return;
    ordersList.innerHTML = '<div class="loading">加载中...</div>';
    try {
        const submitterNameParam = `&submitter_name=${encodeURIComponent(currentUser.name || '')}`;
        const viewModeParam = accessLevel === 'admin' ? '&view_mode=all' : '&view_mode=mine';
        const response = await apiFetch(`${API_BASE}/api/orders?page=${page}&per_page=${PER_PAGE}${submitterNameParam}${viewModeParam}`);
        const data = await response.json();
        if (data.success) {
            historyOrdersCache = data.orders || [];
            historyTotalPages = data.pagination?.total_pages || 1;
            if (historyOrdersCache.length === 0) {
                ordersList.innerHTML = '<div class="empty-state"><div class="empty-state-icon">📋</div><p>暂无历史排队</p></div>';
                return;
            }
            renderHistoryOrders(historyOrdersCache);
            renderHistoryPagination();
            populateHistoryFilterModelSelect();
        } else {
            ordersList.innerHTML = '<div class="empty-state"><div class="empty-state-icon">📋</div><p>加载失败：' + (data.error || '未知错误') + '</p></div>';
        }
    } catch (error) {
        console.error('[loadHistoryOrders] error:', error);
        ordersList.innerHTML = '<div class="empty-state"><div class="empty-state-icon">📋</div><p>网络错误，请检查连接</p></div>';
    }
}
...
function renderHistoryOrders(orders) {
    ...
    let html = '<table class="order-table"><thead><tr><th>型号</th><th>吨位</th><th>客户</th><th>排队日期</th><th>提交时间</th></tr></thead><tbody>';
    orders.forEach(order => {
        let queueDateDisplay = escapeHtml(order.queue_date);
        if (queueDateDisplay && queueDateDisplay.match(/^\d{4}-\d{2}-\d{2}$/) && queueDateDisplay !== '1899-12-30') {
            queueDateDisplay = queueDateDisplay.substring(5);
        } else {
            queueDateDisplay = '';
        }
        let submitTimeDisplay = escapeHtml(order.submit_time);
        if (submitTimeDisplay && submitTimeDisplay.length >= 16) {
            submitTimeDisplay = submitTimeDisplay.substring(5, 16);
        }
        html += '<tr><td class="td-model">' + escapeHtml(order.model) + '</td><td>' + escapeHtml(order.tonnage) + '</td><td>' + escapeHtml(order.customer) + '</td><td>' + queueDateDisplay + '</td><td>' + submitTimeDisplay + '</td></tr>';
    });
    html += '</tbody></table>';
    ordersList.innerHTML = html;
}
...
function renderHistoryPagination() {
    ...
    if (historyCurrentPage > 1) {
        html += '<button onclick="loadHistoryOrders(' + (historyCurrentPage - 1) + ')">上一页</button>';
    } else {
        html += '<button disabled>上一页</button>';
    }
    html += '<span class="page-info">' + historyCurrentPage + ' / ' + historyTotalPages + '</span>';
    if (historyCurrentPage < historyTotalPages) {
        html += '<button onclick="loadHistoryOrders(' + (historyCurrentPage + 1) + ')">下一页</button>';
    } else {
        html += '<button disabled>下一页</button>';
    }
    html += '</div>';
...
function filterHistoryOrders() {
    ...
    const sortType = document.getElementById('historySortSelect').value;
    let filtered = [...historyOrdersCache];
    if (modelFilter) {
        filtered = filtered.filter(o => o.model === modelFilter);
    }
    if (sortType === 'submitTime') {
        filtered.sort((a, b) => (a.submit_time || '').localeCompare(b.submit_time || ''));
    } else if (sortType === 'queueDate') {
        filtered.sort((a, b) => (a.queue_date || '').localeCompare(b.queue_date || ''));
    } else if (sortType === 'model') {
        filtered.sort((a, b) => (a.model || '').localeCompare(b.model || ''));
    } else if (sortType === 'tonnage') {
        filtered.sort((a, b) => parseFloat(a.tonnage || 0) - parseFloat(b.tonnage || 0));
    }
...
function populateHistoryFilterModelSelect() {
    const select = document.getElementById('historyFilterModel');
    if (!select) return;
    const currentVal = select.value;
    const models = [...new Set([...modelOptions, ...historyOrdersCache.map(o => o.model)].filter(Boolean))].sort();
    select.innerHTML = '<option value="">全部型号</option>';
    models.forEach(model => {
        const option = document.createElement('option');
        option.value = model;
...
async function loadPackaging() {
    try {
        const response = await apiFetch(`${API_BASE}/api/packaging`);
        const data = await response.json();
        if (data.success) {
            populatePackagingSelect(data.packaging);
        }
    } catch (error) {
...
function populatePackagingSelect(packagingList) {
    const select = document.getElementById('packaging');
    if (!select) return;
...
window.showTab = function(tabName, evt) {
    ...
    if (tabName === 'history') loadHistoryOrders(1, true);
};

// 在 DOMContentLoaded 中加载包装数据
document.addEventListener('DOMContentLoaded', function() {
    if (typeof loadPackaging === 'function') {
        setTimeout(loadPackaging, 1000);
    }
});
