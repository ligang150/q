...
let cachedHistoryOrders = [];   // 历史排队缓存
...
let viewMode = 'mine'; // 'mine' 或 'all' 或 'history'
...
function clearOrderForm() {
    ...
    if (calc) calc.value = '';
    const pkgReset = document.getElementById('packaging');
    if (pkgReset) pkgReset.value = '';
    pendingRowIndex = 0;
}
...
async function loadPackagingOptions() {
    try {
        ...
        const data = await response.json();
        if (data.success && Array.isArray(data.packaging)) {
            packagingOptions = data.packaging;
            const select = document.getElementById('packaging');
            if (select) {
                const currentVal = select.value;
                select.innerHTML = '<option value="">请选择包装</option>';
            ...
            const editSelect = document.getElementById('editPackaging');
            if (editSelect) {
                const currentVal = editSelect.value;
                editSelect.innerHTML = '<option value="">请选择包装</option>';
                packagingOptions.forEach(opt => {
                    const option = document.createElement('option');
                    option.value = opt;
                    option.textContent = opt;
        ...
        console.warn('[loadPackagingOptions] error:', error);
    }
}
...
function setupEventListeners() {
    ...
    // 监听表单字段变化，记录草稿
    const draftFields = ['model', 'tonnage', 'customer', 'expectedDate', 'queueDate', 'packaging'];
...
async function handleCreateOrder(e) {
    ...
    const packaging = packagingInput ? packagingInput.value : '';
    
    if (!packaging) {
        showToast('请选择包装', 'error');
        return;
    }
...
async function loadOrders(page = 1, forceRefresh = false, options = {}) {
    ...
    try {
        ...
        if (data.success) {
            ...
            } else if (requestViewMode === 'history') {
                cachedHistoryOrders = newOrders;
            } else {
                cachedAllOrders = newOrders;
            }
            // 如果API返回空但对应缓存有数据，使用对应缓存（降级显示）
            let cachedOrders = requestViewMode === 'mine' ? cachedMineOrders : (requestViewMode === 'history' ? cachedHistoryOrders : cachedAllOrders);
            ...
            let cachedOrders = viewMode === 'mine' ? cachedMineOrders : (viewMode === 'history' ? cachedHistoryOrders : cachedAllOrders);
...
function renderOrders(orders) {
    ...
    // 权限判断：业务员（非管理员非经理）在全部排队/历史排队隐藏客户
    const isHistoryView = viewMode === 'history';
    const canSeeCustomer = viewMode === 'mine' || isAdmin || isManager;
    // 历史排队禁止修改、删除
    const canOperate = !isHistoryView && (viewMode === 'mine' || isAdmin || isManager);
    const showSubmitter = viewMode === 'all' || isHistoryView;
    ...
    orders.forEach(order => {
        ...
        html += `<tr>
            ...
            ${showSubmitter ? '<td>' + escapeHtml(order.submitter || '') + '</td>' : ''}
...
function renderAdminFilter() {
    ...
    const historyClass = viewMode === 'history' ? 'active' : '';
    ...
    filterEl.innerHTML = `
        <div class="admin-filter">
            ...
            <button class="${historyClass}" onclick="switchViewMode('history')">历史排队</button>
        </div>
    `;
}
...
function switchViewMode(mode) {
    ...
    if (mode === 'history' && sortEl) {
        sortEl.value = 'submitTime';
    }
    loadOrders(1, true);
}
    ...
    try {
        ...
        if (data.success) {
            ...
            if (order) {
                ...
                const editPkg = document.getElementById('editPackaging');
                if (editPkg) {
                    // 动态添加选项（防止包装选项列表未及时加载）
                    if (order.packaging && !Array.from(editPkg.options).some(o => o.value === order.packaging)) {
                        const opt = document.createElement('option');
                        opt.value = order.packaging;
                        opt.textContent = order.packaging;
                        editPkg.appendChild(opt);
                    }
                    editPkg.value = order.packaging || '';
                }
...
async function copyOrder(rowIndex) {
    try {
        ...
        document.getElementById('calculatedDate').value = '';
        document.getElementById('queueDate').value = '';
        const pkgCopy = document.getElementById('packaging');
        if (pkgCopy) {
            if (order.packaging && !Array.from(pkgCopy.options).some(o => o.value === order.packaging)) {
                const opt = document.createElement('option');
                opt.value = order.packaging;
                opt.textContent = order.packaging;
                pkgCopy.appendChild(opt);
            }
            pkgCopy.value = order.packaging || '';
        }
...
async function handleUpdateOrder(e) {
    ...
    const orderData = {
        ...
        queue_date: queueDate,
        packaging: document.getElementById('editPackaging') ? document.getElementById('editPackaging').value : '',
        submitter: currentUser.name,
        submitter_id: currentUser.id
    };
